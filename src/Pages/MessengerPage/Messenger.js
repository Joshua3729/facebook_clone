...
const MessengerPage = () => {
  ...
  const chats_data = useSelector((state) => state.messenger.chats_data);
  const messages_data = useSelector((state) => state.messenger.messages_data);
  const user_data = useSelector((state) => state.messenger.user_data);
  const new_message = useSelector((state) => state.messenger.user_message);
  const token = useSelector((state) => state.auth.token);
  const user_id = useParams().user_id;
  const sender_user_id = useSelector((state) => state.auth.user_data.user_id);

  const dispatch = useDispatch();

  useEffect(() => {
    document.body.style.overflowY = "scroll";
    dispatch(HomeActions.getChats(token));
    dispatch(HomeActions.getMessages(user_id, token));
  ...
  const openMessages = (id) => {
    ...
    dispatch(HomeActions.getMessages(id, token));
  };
        ...
        <div className={classes.message_wrapper}>
          ...
          >
            ...
            <input
              ...
              value={new_message}
              onChange={(e) =>
                dispatch(HomeActions.onMessageChange(e.target.value))
              }
            />
          </form>
        </div>
